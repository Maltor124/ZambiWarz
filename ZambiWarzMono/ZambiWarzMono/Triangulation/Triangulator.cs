...
namespace ZambiWarzMono
{
    class Triangulator
    {
        ...
        private void Analyse(List<Vector2> suppliedPoints, Hull hull, List<Triad> triads, bool rejectDuplicatePoints, bool hullOnly)
        {
            ...
            {
                distance2ToCentre[k] = Vector2.DistanceSquared(points[0], points[k]);
                sortedIndices[k] = k;
            }
            ...
            for (int k = 3; k < nump; k++)
                distance2ToCentre[k] = Vector2.DistanceSquared(points[sortedIndices[k]], centre);
                ...
                if (hull.EdgeVisibleFrom(0, dx, dy))
                {
                    ...
                    if (hull.EdgeVisibleFrom(numh - 1, dx, dy))
                    {
                        ...
                        for (int h = 0; h < numh - 1; h++)
                        {
                            ...
                            if (hull.EdgeVisibleFrom(h, new Vector2(ptx.X, ptx.Y)))
                            {
                                hull.RemoveAt(h);
                                h--;
                                numh--;
        ...
        {
            int oppositeVector2 = 0, edge1, edge2, edge3 = 0, edge4 = 0;
            triadIndexFlipped = 0;
            ...
            if (tri.bc >= 0)
            {
                ...
                t2.FindAdjacency(tri.b, triadIndexToTest, out oppositeVector2, out edge3, out edge4);
                if (tri.InsideCircumcircle(points[oppositeVector2]))
                {  // not valid in the Delaunay sense.
                    ...
                    if (edge1 != edge3 && edge2 != edge4)
                    {
                        ...
                        tri.Initialize(tria, trib, oppositeVector2, edge1, edge3, triadIndexFlipped, points);
                        t2.Initialize(tria, tric, oppositeVector2, edge2, edge4, triadIndexToTest, points);
            ...
            if (tri.ab >= 0)
            {
                ...
                Triad t2 = triads[triadIndexFlipped];
                // find relative orientation (shared limb).
                t2.FindAdjacency(tri.a, triadIndexToTest, out oppositeVector2, out edge3, out edge4);
                if (tri.InsideCircumcircle(points[oppositeVector2]))
                {  // not valid in the Delaunay sense.
                    edge1 = tri.ac;
                    edge2 = tri.bc;
                    ...
                    {
                        int tria = tri.a, trib = tri.b, tric = tri.c;
                        tri.Initialize(tric, tria, oppositeVector2, edge1, edge3, triadIndexFlipped, points);
                        t2.Initialize(tric, trib, oppositeVector2, edge2, edge4, triadIndexToTest, points);
            ...
            if (tri.ac >= 0)
            {
                ...
                {  // not valid in the Delaunay sense.
                    ...
                    {
                        int tria = tri.a, trib = tri.b, tric = tri.c;
                        tri.Initialize(trib, tria, oppositeVector2, edge1, edge3, triadIndexFlipped, points);
                        t2.Initialize(trib, tric, oppositeVector2, edge2, edge4, triadIndexToTest, points);
